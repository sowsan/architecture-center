...
| --- | --- | --- |
| [Owner](/azure/role-based-access-control/built-in-roles#owner) |Managed ID from Contoso's Active Directory |This ID is controlled with Just in Time (JIT) access through Contoso's Identity Management tool and ensures that subscription owner access is fully audited |
